...
const NonActive = (title: string) => (
  <div key="non" className="absolute z-0 lg:bottom-20 lg:w-[170px]">
    <h3 className="lg:-rotate-90 lg:origin-center font-semibold text-[18px] sm:text-[26px] text-white">
      {title}
    </h3>
  </div>
);
...
const ActivePanel = (title: string) => (
  <motion.div
    key="active"
    variants={slideIn(0, '100%', 'tween', 0.5, 0.3)}
    initial="hidden"
    animate="show"
  ...
  >
    ...
    <h2 className="mt-[24px] text-[24px] sm:text-[32px] font-semibold text-white">{title}</h2>
  </motion.div>
);
