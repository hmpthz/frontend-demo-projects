...
export function Header() {
  ...
  return (
    <div
      id="header"
      className={`fixed top-0 left-0 w-full z-50 border-b border-n-6 lg:backdrop-blur-sm ${isMenuOpned ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'}`}
    >
...
const Navigations = (path: string, isMenuOpned: boolean, setMenuOpened: ReactSetState<boolean>) => (
  <>
    <nav
      className={`${isMenuOpned ? 'flex' : 'hidden'} lg:flex fixed lg:static top-20 left-0 right-0 bottom-0 bg-n-8 lg:bg-transparent lg:mx-auto`}
    >
    ...
    <a
      href="#signup"
      className="hidden lg:block button text-n-1/50 transition-colors hover:text-n-1 mr-8"
    >
      New account
    </a>
...
const NavLink = (item: NavItem, path: string, onClick: () => void) => (
  <a
    ...
    href={item.url}
    onClick={onClick}
    className={`block ${item.onlyMobile ? 'lg:hidden' : ''} font-code text-2xl lg:text-sm uppercase text-n-1 ${item.url == path ? 'lg:text-n-1' : 'lg:text-n-1/50'} transition-colors hover:text-color-1 lg:hover:text-n-1 px-6 xl:px-12 py-6 md:py-8 lg:font-semibold lg:leading-5`}
  >
    {item.title}
  </a>
);
