...
const Block1 = () => (
  <div className="relative w-full flex items-center h-[39rem] xl:h-[46rem] mb-5 p-8 lg:p-20 border border-n-1/10 rounded-3xl overflow-hidden">
    <img
      src={service1}
      className="absolute z-0 top-0 left-0 w-full md:w-3/5 xl:w-auto h-full object-cover md:object-right pointer-events-none"
      alt="Smartest AI"
    />
...
const Block2 = () => (
  <div className="relative min-h-[39rem] mb-5 border border-n-1/10 rounded-3xl overflow-hidden">
    <img
      src={service2}
      className="absolute z-0 top-0 left-0 w-full h-full object-cover pointer-events-none"
      alt="Robot"
    />
...
const ServiceIcon = (item: string, index: number) => (
  <li
    key={index}
    className={`rounded-2xl ${index == 2 ? 'w-12 h-12 md:w-[4.5rem] md:h-[4.5rem] p-0.5 bg-conic-gradient' : 'w-10 h-10 md:w-15 md:h-15 bg-n-6'}`}
  >
    <div
      className={`flex h-full justify-center items-center ${index == 2 ? 'bg-n-7 rounded-2xl' : ''}`}
    >
